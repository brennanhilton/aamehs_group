...
final_data_2017_ela =
  final_data %>%
  filter(year == "2017") %>%
  select(percent_elevated, ela_score, percent_white, percent_asian, percent_black, percent_hispanic, economic_need_index, percent_english_language_learners)

final_data_2017_math =
  final_data %>%
  filter(year == "2017") %>%
  select(percent_elevated, math_score, percent_white, percent_asian, percent_black, percent_hispanic, economic_need_index, percent_english_language_learners)
...
df.dist_2016_ela <- dist(final_data_2016_ela, method = "euclidean")
df.dist_2016_math <- dist(final_data_2016_math, method = "euclidean")

df.dist_2017_ela <- dist(final_data_2017_ela, method = "euclidean")
df.dist_2017_math <- dist(final_data_2017_math, method = "euclidean")
...
hc.complete_2016_ela <- hclust(df.dist_2016_ela , method = "complete")
hc.complete_2016_math <- hclust(df.dist_2016_math, method = "complete")

hc.complete_2017_ela <- hclust(df.dist_2017_ela, method = "complete")
hc.complete_2017_math <- hclust(df.dist_2017_math, method = "complete")

# 6b Look at dendrogram

as.dendrogram(hc.complete_2016_math) %>% head()

as.dendrogram(hc.complete_2017_math) %>% head()
```
...
dendro.complete_2016_ela <- as.dendrogram(hc.complete_2016_ela)
dendro.complete_2016_math <- as.dendrogram(hc.complete_2016_math)

dendro.complete_2017_ela <- as.dendrogram(hc.complete_2017_ela)
dendro.complete_2017_math <- as.dendrogram(hc.complete_2017_math)
...
dendro.complete_2016_ela %>% 
  plot(main = "Complete Linkage 2016 ELA Scores", ylab = "Height", leaflab = "none")
dendro.complete_2016_math %>% 
  plot(main = "Complete Linkage 2016 Math Scores", ylab = "Height", leaflab = "none")

dendro.complete_2017_ela %>% 
  plot(main = "Complete Linkage 2017 ELA Scores", ylab = "Height", leaflab = "none")
dendro.complete_2017_math %>% 
  plot(main = "Complete Linkage 2017 Math Scores", ylab = "Height", leaflab = "none")
```
...
#2017
dendro.complete_2017_ela  %>% #selected this one 
  color_branches(k = 7) %>%
  plot(main = "Complete Linkage 2017 ELA Scores", ylab = "Height", leaflab = "none") %>%
abline(h = 16, lty = 3)
...
```{r}
# 8c.i Cut the tree to have 7 clusters

hc.cluster.7 <- cutree(hc.complete_2016_ela, 7)
...
df.hc <- final_data_2016_ela %>% 
  mutate(hcluster.7 = hc.cluster.7) %>%
  rename("% Elevated Pb" = "percent_elevated",
...
plot_means_h7 <- df.hc.mean %>%
  gather(key = "group", value = "mean", -hcluster.7) 
...
ggplot(plot_means_hc7.7c, aes(x = group, y = mean, fill = group)) +
  geom_col() +
  geom_hline(yintercept = 0, size = 0.2) +
  facet_wrap(~ hcluster.7) +        # creates 7 plots, for each cluster
  theme_bw() +  theme(axis.text = element_text(angle = 90))
```

#do this separately for math and ela scores!! 